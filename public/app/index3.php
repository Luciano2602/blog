...
<?php 
    ...
    if ( !isset($_SESSION["user_portal"])){
        ...
        $idAutor = $_SESSION["user_portal"];
    }
        
    
    //consulta ao banco de dados
    $consulta = "select id_post, p.id_autor, titulo, sub_titulo, noticia, imagem,fraseAutor,textoCurto, nome, sobre_nome ";
    $consulta .= "from post ";
    $consulta .= "p inner join autor a on a.id_autor = p.id_autor ";

    if(isset($_GET["filtro"])){
        $nomeFiltro = urlencode($_GET["filtro"]);
        $consulta .= "where titulo like '%{$nomeFiltro}%' and p.id_autor = {$idAutor} ";
    }
   ...
   //$consulta .= "limit 36";
    ...
    $autor = "select nome from autor where id_autor = {$idAutor}";
    $resultAutor = mysqli_query($conexao, $autor);
    
    if(!$resultAutor){
        die("Falha na consulta");
    }
    
    while ($li = mysqli_fetch_assoc($resultAutor)){
        $nomeAutor = $li['nome'];
    }
    ...
    <body>
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="elementoCollapsel">  
                    <ul class="nav navbar-nav">
                        ...
                        <li><a href="cadastrarPost.php">Cadastrar</a></li>
                        <spam>
                            <?php echo "olá " . $nomeAutor ?>
                        </spam>
                    </ul>
                                        
                    <form class="navbar-form navbar-right" role="search" action="index3.php" method="get">
        ...
        <div class="row" id="autores">  
            ...
            <div class="col-md-4 text-center">
                ...
                <a href="alterar.php?id_post=<?php echo $linha['id_post']; ?>" class="btn btn-warning">Alterar</a>
                <a href="excluir.php?id_post=<?php echo $linha['id_post'];?>" class="btn btn-danger">Excluir</a>
			</div>
            <?php
                }  //fim do while
            ?>
